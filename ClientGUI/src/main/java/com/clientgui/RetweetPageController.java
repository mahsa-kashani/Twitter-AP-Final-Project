...
public class RetweetPageController implements Initializable {
    String userName;
    String retweetTo;
    Stage stage;
    ...
    public void tweetPressed() throws IOException {
        ...
        if(selectedPhoto.equals(""))
            Main.tweet(text.getText() , null , retweetTo , "" , userName);
        else
            Main.tweet(text.getText() , Files.readAllBytes(Path.of(selectedPhoto)) , retweetTo , "" , userName);
        stage.close();
        mainPage.refresh();
    }
    Stage getMyStage(){
